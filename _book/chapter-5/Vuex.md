...
分发自定义事件而非在 *ChangeTitleComponent* 内输入， 在父级组件内监听这个事件：
...
好的， 现在我们有了 actions。 在我们的 *ChangeGreetingsComponent* 组件内使用！ 为了在组件内使用 actions， 我们应该先导入它们到仓库然后再以新的 *Vuex* 对象导出。 然后 actions 可以 *this.$store.dispatch* 方法在组件内分发：
